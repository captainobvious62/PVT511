...
PROGRAM main
  ...
  !===========================================================================80
  ! Try the nonsense again without PETSc
  !---------------------------------------------------------------------------80

  CALL PrintMsg("Initial Guess")    
  solution(1) = x_init(1)   ! A
  solution(2) = x_init(2)   ! B
  CALL FindR_AB(solution(1),solution(2),z_cnplus,MW_cnplus,c_min,c_max,residual)

  ! Crappy Jacobian Calc
  jacobian(:,:) = 0.0
  ! A+
  CALL FindR_AB(solution(1)+eptol,solution(2),z_cnplus,MW_cnplus,c_min,c_max,FD_E)
  ! A-
  CALL FindR_AB(solution(1)-eptol,solution(2),z_cnplus,MW_cnplus,c_min,c_max,FD_W)
  jacobian(:,1) = (FD_E - FD_W) / 2.*eptol
...
!  CALL VecDuplicate(Vec_v,Vec_RHS,ierr)          
!  CALL VecDuplicate(Vec_v,Vec_dV,ierr)        
!  do i=1,5
...
!      
!      ! Analytical Test
!      CALL AnalyticalJacobian(Vec_v,Mat_J,ierr)
!      
!      ! Form RHS

!      CALL VecCopy(Vec_r,Vec_RHS,ierr)
!      CALL VecScale(Vec_RHS,-f1,ierr)

!      ! Solve linear system for dV

!      CALL KSPSolve(Krylov,Vec_RHS,Vec_dV,ierr)
!      CALL VecView(Vec_dV,PETSC_VIEWER_STDOUT_SELF,ierr)
!      CALL PrintMsg("dV")
!      CALL PrintMsg(" ")      

!!      ! Update residual vector
!      CALL VecAXPY(Vec_v,f1,Vec_dV,ierr)
