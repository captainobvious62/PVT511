...
VF,x,y = VLE(R,T,P,Pc,Tc,z,omega,kij,eptol,maxiter)

Zc_x,Zc_y,K_l,K_v,S_l,S_v,v_trivial,l_trivial,x,y = StabTest(R,P,T,z,Pc,Tc,omega,kij,maxiter,gdem_interval)
  
      
#def TwoPhase(z,P,T,Pc,Tc,omega,kij):
K = WilsonEqn(Pc,Tc,omega,T,P)      
# Compute Vapor Phase
v = K*z      
...
#numvals = 1501
#numtemps = 11
#T_data = np.linspace(0 + 273.15,100+273.15,numtemps) # K
#P_data = np.linspace(1,200,numvals)

#x_data = np.zeros([numvals,z.size])
#y_data = np.zeros([numvals,z.size])
#VF_data = np.zeros([numvals,numtemps])

#for i in np.arange(0,numvals):
#    for j in np.arange(0,numtemps):
#        VF_data[i,j],x_data[i,:],y_data[i,:] =  VLE(R,T_data[j],P_data[i],Pc,Tc,z,omega,kij,eptol,maxiter)

#fig, ax = plt.subplots()
#color = cm.rainbow(np.linspace(0,1,numtemps))
#for i,c in zip(range(numtemps),color):
#    ax.plot(P_data,VF_data[:,i],c=c)


#ax.set_title('VF vs. Pressure at Temperatures')
...
Pmax = 500.
Pnum = 51
Prange = np.linspace(Pmin,Pmax,Pnum)
...
TPGrid = np.ones([Pnum,Tnum],dtype=bool)

for i in np.arange(0,Pnum):
    for j in np.arange(0,Tnum):
        TPGrid[j,i] = tpdss(z,Prange[i],Trange[j],Pc,Tc,omega,kij,1e-6,50)
